...
const ApiUtils = {
  ...
  async fetchItemIdsFromServer() {
    try {
      ...
      const response = await fetch(
        `${STATE.jellyfinData.serverAddress}/Items?imageTypes=Primary&sortBy=PremiereDate&sortOrder=Descending&enableUserData=true&Limit=${CONFIG.maxItems}&fields=Id`,
        {
          headers: this.getAuthHeaders(),
        }
      );
